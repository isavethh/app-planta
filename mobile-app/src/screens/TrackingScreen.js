...
export default function TrackingScreen({ route, navigation }) {
  ...
  const handleSimularRuta = async () => {
    try {
      ...
      // Si el backend devuelve puntos (aunque no pueda persistirlos en BD), úsalos inmediatamente
      if (resp && resp.puntos && Array.isArray(resp.puntos) && resp.puntos.length > 0) {
        setTracking(resp.puntos);
      }
      ...
      // Intentar recargar desde el endpoint de seguimiento (si la BD guardó los puntos)
      cargarDatos();
    } catch (error) {
